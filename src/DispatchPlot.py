...
try:
  from ravenframework.PluginBaseClasses.OutStreamPlotPlugin import (
    InputData,
    InputTypes,
    PlotPlugin,
  )
...
class DispatchPlot(PlotPlugin):
  ...
  def getInputSpecification(cls):
    ...
    specs.addSub(
      InputData.parameterInputFactory("source", contentType=InputTypes.StringType)
    )
    specs.addSub(
      InputData.parameterInputFactory(
        "macro_variable", contentType=InputTypes.StringType
      )
    )
    specs.addSub(
      InputData.parameterInputFactory(
        "micro_variable", contentType=InputTypes.StringType
      )
    )
    specs.addSub(
      InputData.parameterInputFactory("signals", contentType=InputTypes.StringListType)
    )
    return specs
  ...
  def initialize(self, stepEntities):
    ...
    if src is None:
      self.raiseAnError(
        IOError, f'Source DataObject "{self._sourceName}" was not found in the Step!'
      )
    self._source = src
  ...
  def plot_dispatch(self, figs, axes, df, grp_vars, sid, mstep, cid, cdict) -> None:
    ...
    for f, fig in enumerate(figs):
      ...
      fig.suptitle(
        f"Dispatch ID {sid} Year {mstep} Cluster {cid},\nFigure {f+1}/{len(figs)}"
      )
      fig.tight_layout()
  ...
  def plot_signal(self, fig, axes, df, sid, mstep, cid) -> None:
    ...
    for name, ax in zip(self._addSignals, axes.flat):
      ...
      if var is None:
        self.raiseAnError(
          RuntimeError, f'Requested signal variable "{name}" but variable not in data!'
        )
  ...
  def color_style(self, grp_vars):
    ...
    if n_comps > n_uniques:
      self.raiseAWarning(
        f"A total of {n_comps} exist to plot, but only {n_uniques} unique identifying "
  ...
  def run(self):
    ...
    for sample_id, macro_step, cluster_id in it.product(sample_ids, macro_steps, cluster_ids):
      ...
      self.plot_dispatch(
        res_figs, res_axs, dat, grouped_vars, sample_id, macro_step, cluster_id, cdict
      )

      # Output synthetic time series signal for current dimension.
      sig_figs, sig_axs = plt.subplots(
        len(self._addSignals), 1, sharex=True, squeeze=False
      )
