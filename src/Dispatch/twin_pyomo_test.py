...
def make_concrete_model():
  ...
  m.steam_source_lower_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.steam_source_production[0, t] >= steam_produced
  )
  m.steam_source_upper_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.steam_source_production[0, t] <= steam_produced
  )
  ...
  m.elec_generator_lower_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.elec_generator_production[0, t] >= -gen_consume_limit
  )
  m.elec_generator_upper_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.elec_generator_production[0, t] <= 0
  )
  # elec sink can take any amount of electricity
  # -> consuming, so -10000 < consumed elec < 0
  m.elec_sink_lower_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.elec_sink_production[0, t] >= -sink_limit
  )
  m.elec_sink_upper_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.elec_sink_production[0, t] <= 0
  )
  # storage is in LEVEL not ACTIVITY (e.g. kg not kg/s) -> lets say it can store X kg
  m.steam_storage_lower_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.steam_storage_production[0, t] >= 0
  )
  m.steam_storage_upper_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.steam_storage_production[0, t] <= storage_limit
  )
  # *******************
...
def _conserve_steam(m, t):
  ...
  storage_source = (
    -(
      m.steam_storage_production[0, t]
      - (storage_initial if t == 0 else m.steam_storage_production[0, t - 1])
    )
    / dt
  )
