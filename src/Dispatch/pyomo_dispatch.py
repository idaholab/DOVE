...
class Pyomo(Dispatcher):
  ...
  def _handle_dispatch_window_solve(
    self,
    specific_time,
    start_index,
    case,
    ...
    for comp in components:
      ...
      if intr.is_governed():  # by "is_governed" we mean "isn't optimized in pyomo"
        ...
        if scale == 0:
          ...
          diff = (
            np.linalg.norm(new[name][tracker][res] - old[name][tracker][res]) / scale
          )
        if diff > tol:
          return False
    return True
  ...
  def _solve_dispatch(self, m, meta):
    ...
    while not done_and_checked:
      ...
      # check solve status
      if (
        soln.solver.status == SolverStatus.ok
        and soln.solver.termination_condition == TerminationCondition.optimal
      ):
      ...
      if validation_errs:
        ...
        for e in validation_errs:
          print(
            f"DEBUGG ... ... Time {e['time_index']} ({e['time']}) \n"
