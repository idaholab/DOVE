...
class Custom(Dispatcher):
  ...
  def read_input(self, inputs):
    ...
    if usr_loc is None:
      raise RuntimeError(
        "No <location> provided for <custom> dispatch strategy in <Case>!"
      )
  ...
  def initialize(self, case, components, sources, **kwargs):
    ...
    if not os.path.isfile(file_loc):
      raise IOError(
        f'Custom dispatcher not found at "{file_loc}"! (input dir "{start_loc}", provided path "{self._usr_loc}"'
      )
    self._file = file_loc
    ...
    if "dispatch" not in dir(module):
      raise IOError(
        f'Custom Dispatch at "{self._file}" does not have a "dispatch" method!'
      )
    # TODO other checks?
  ...
  def dispatch(self, case, components, sources, meta):
    ...
    time = np.linspace(
      t_start, t_end, t_num
    )  # Note we don't care about segment/cluster here
    # load user module
    load_string, _ = utils.identifyIfExternalModelExists(self, self._file, "")
    module = utils.importFromPath(load_string, True)
    state = NumpyState()
