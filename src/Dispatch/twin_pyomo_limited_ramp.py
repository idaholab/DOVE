...
def make_concrete_model():
  ...
  m.NPP_upper_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.NPP_production[0, t] <= steam_produced
  )
  ...
  m.BOP_lower_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.BOP_production[0, t] >= -gen_consume_limit
  )
  ...
  m.Grid_lower_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.Grid_production[0, t] >= -sink_limit
  )
  m.Grid_upper_limit = pyo.Constraint(
    m.T, rule=lambda m, t: m.Grid_production[0, t] <= 0
  )
  # *******************
...
def _ramp_up_NPP(m, t):
  ...
  if t > 0:
    ineq = (
      m.NPP_production[0, t] - m.NPP_production[0, t - 1]
      <= ramp_up_limit * m.ramp_up[t] - delta * m.ramp_down[t]
    )
  else:
...
def _ramp_down_NPP(m, t):
  ...
  if t > 0:
    ineq = (
      m.NPP_production[0, t - 1] - m.NPP_production[0, t]
      <= ramp_down_limit * m.ramp_down[t] - delta * m.ramp_up[t]
    )
  else:
    ineq = pyo.Constraint.Skip
  return ineq
...
def _economics(m):
  ...
  opex = (
    sum(m.BOP_production[0, t] for t in m.T) * margin_NPP
  )  # will be negative b/c consumed
  # grid sales, calculated above; negative sign because we're consuming
  sales = -sum(
    (m.Grid_production[0, t] * elec_price[t]) for t in m.T
