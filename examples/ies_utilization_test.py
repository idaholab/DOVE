...
def create_system(solar_capacity_scale: float):
    ...
    solar = dc.Source(
        name="solar",
        produces=elec,
        installed_capacity=solar_capacity_scale,
    ...
    ng = dc.Source(
        ...
        produces=elec,
        installed_capacity=20 - solar_capacity_scale * np.average(solar_cap_factor_profile),
        cashflows=[
    ...
    sys = dc.System(
        ...
        dispatch_window=list(range(len(grid_profile))),
    )
...
def run_test():
    ...
    for solar_cap_multiplier in range(4, 20, 4):
        results[f"{solar_cap_multiplier}_GW_solar_capacity"] = create_system(solar_cap_multiplier)
