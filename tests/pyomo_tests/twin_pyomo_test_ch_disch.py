...
def make_concrete_model():
  ...
  m.elec_generator_production = pyo.Var(
    m.elec_generator_index_map, m.T, initialize=0, bounds=(-gen_consume_limit, 0)
  )
  # steam_storage
...
def _conserve_steam(m, t):
  ...
  storage_source = (
    -(
      m.steam_storage_level[0, t]
      - (
        storage_initial
        if t == 0
        else (m.steam_storage_charge[0, t - 1] + m.steam_storage_discharge[0, t - 1])
      )
    )
    / dt
  )
